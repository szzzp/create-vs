...
const { vueRepo, vuehPepo } = require("../config/repo-config.js");
...
const createProjectAction = async (project) => {
  ...
  try {
    ...
    console.log('依赖包下载错误' + error);
  }
};

// 创建移动模板
const createHProjectAction = async (project) => {
  console.log('开始下载移动模板(The mobile template starts to be downloaded)');

  try {
    await download(vuehPepo, project, { clone: true })
  } catch (error) {
    console.log('下载模板错误' + error);
  }
